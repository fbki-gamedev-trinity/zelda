...
func add_item(item: InventoryItem, stacks: int):
	...
	else: # логика вставания предмета именно в пустое место в инвентаре
...
func eject_item_into_the_ground(idx: int):
	...
	var inventory_item_to_eject = items[idx]#предмет который выкидываем
	# делаем его подбираемым
	var item_to_eject_as_pickup = PICKUP_ITEM_SCENE.instantiate() as PickUpItem
	
	item_to_eject_as_pickup.inventry_item = inventory_item_to_eject
	item_to_eject_as_pickup.stacks = inventory_item_to_eject.stacks
	
	get_tree().root.add_child(item_to_eject_as_pickup)
	
	item_to_eject_as_pickup.disable_collision() # чтобы сразу не подбирался
	item_to_eject_as_pickup.global_position = get_parent().global_position
	
	#настраиваем выброс туда куда смотрим в определенном радиусе
	...
	ejection_tween.finished.connect(func(): item_to_eject_as_pickup.enable_collision()) #возвращаем поднимаемость
	
	# убираем выкинутый предмет из атаковой системы
